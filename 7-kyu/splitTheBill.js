...
function splitTheBill(x) {
  let totalSpent = Object.values(x).reduce((acc, val) => acc + val, 0);
  
  let fairShare = totalSpent / Object.keys(x).length;
  
  let result = {};
  for (let person in x) {
    result[person] = parseFloat((x[person] - fairShare).toFixed(2));
  }
