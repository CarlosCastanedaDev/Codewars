...
function leaderB(user, user_score, your_score){
    ...
    let beta = parseInt(diff/3);
    let kyu8 = diff % 3;
    let phrase = `To beat ${user}'s score, I must complete ${beta} Beta kata and ${kyu8} 8kyu kata.`;
    if(diff < 0) return 'Winning!';
    if(diff === 0) return 'Only need one!';
    
    return beta + kyu8 > 1000 ? phrase + ' Dammit!' : phrase
  };
